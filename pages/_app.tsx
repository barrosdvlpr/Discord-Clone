  ...
  <div className="flex h-screen ">
    <div className="bg-gray-900 text-gray-100 min-h-screen p-3 space-y-2 overflow-y-scroll min-w-min">
      <Link legacyBehavior href="/">
        <a className={`
        ${router.pathname === "/" ? "rounded-2xl bg-brand text-white" : "hover:rounded-2xl hover:bg-brand hover:text-white text-gray-100 bg-gray-700"}
          w-12 h-12 rounded-3xl  flex items-center justify-center transition-all duration-200`}
        ><DiscordIcon className="w-7 h-5 "/></a>
      </Link>
      <Link legacyBehavior href="/servers/1">
        <a className={`bg-gray-700 text-gray-100 w-12 h-12 rounded-3xl hover:rounded-xl flex items-center justify-center hover:bg-brand hover:text-white transition-all duration-200`}
        >S1</a>
      </Link>
    </div>
    <Component {...pageProps}/>
